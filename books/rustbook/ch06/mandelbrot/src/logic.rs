// セルの発散判定を行い、判定までのイテレーション関数を返す関数
...
// 各セルについての色情報を格納する関数
// lib.rs から呼び出すため、pubキーワードをつけてファイル外から参照できるようにする
pub fn generate_mandelbrot_set(
    ...
    // JavaScriptの8ビット符号なし整数の配列であるUint8ClampedArray型を、つくりたいため、Vec<u8>で色情報を作る
    let mut data = vec![];
    for i in 0..canvas_h {
        let i_f64 = i as f64;
        ...
        for j in 0..canvas_w {
            ...
            let v = iter_index % 8 * 32; // 8色にｍ塗り分け
            data.push(v); // R
            data.push(v); // G
            data.push(v); // B
            data.push(255); // A
